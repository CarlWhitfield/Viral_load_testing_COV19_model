...
## Arguments: 
`v` = Container for generated viral load trajectory,
                      interpolated to one value per day

`inf` = Container for generated infectivity,
                      interpolated to one value per day

## Returns: 
`Float64` = time of symptom onset in days (since infection)

`Float64` = peak viral load (log10 copies/ml)

`Bool` = True if asymptomatic (i.e. will show no symptoms
              at generated symptom onset time), and false
              otherwise
...
function build_viral_load_distribution!(v::Array{Float64,1}, inf::Array{Float64,1})
    ...
    resize!(v, Int64(ceil(OT + PT + DT + 10)))
    v .= 0
    resize!(inf, Int64(ceil(OT + PT + DT + 10)))
    inf .= 0
...
`sim` = Dict container with the following keys:
    ...
    "symp_time"=>Array{Float64,1} = Each entry i is the symptom
...
function build_viral_load_distributions!(sim::Dict)
    out = build_viral_load_distribution!.(sim["VL_profiles"], sim["infection_profiles"])
    for n in 1:sim["Ntot"]
        sim["symp_time"][n] = out[n][1]
        sim["VL_mag"][n] = out[n][2]
        sim["asymptomatic"][n] = out[n][3]
    end
end
...
function init_VL_and_infectiousness(Ntot::Int, Pisol::Float64)
    ...
    sim = Dict("Ntot"=>Ntot, "symp_time"=>-ones(Float64, Ntot),
